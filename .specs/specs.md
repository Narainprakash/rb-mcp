...
2. ~~**At beta launch, Agentic Trading supports equities only.**~~ **RESOLVED 2026-07-25 — options ARE supported.** Verified against the live MCP (`hermes mcp test robinhood`): 52 tools exposed over HTTP with OAuth 2.1 PKCE, including `get_option_quotes`, `get_option_instruments`, `get_option_chains`, `place_option_order`, `cancel_option_order`, `get_option_orders`, and `get_option_positions`. The options flow is therefore no longer blocked on Robinhood's roadmap.

   Two consequences worth carrying forward:
   - **`review_option_order` exists** — "simulate an options order without placing it", returning the broker's own pricing and validation. This is a materially better paper mode than a local simulator: it exercises real quotes, real rejections, and real margin/level checks with no money at risk. Paper mode should move onto it once the provider lands, at which point the 1–2 week validation period in Section 10 finally means something.
   - **`get_accounts`** supplies the `account_number` that the order tools require, which is what `users.robinhood_account_id` (Section 16) is for. It must be populated before any live multi-user routing.
...
**How the trading process can reach the MCP — findings so far.** The Hermes Agent holds the Robinhood OAuth session; the trading bot is a separate process and cannot borrow it implicitly. Routes investigated:

- ~~Shell out to the Hermes CLI, as the notifier does with `hermes send`~~ — **ruled out (checked 2026-07-25)**. `hermes mcp` exposes only connection management (`add`, `remove`, `list`, `test`, `configure`, `login`, `reauth`, `picker`, `catalog`, `install`, `serve`); there is no subcommand that invokes a tool on a connected server. `serve` runs Hermes *as* an MCP server for other agents, which is the opposite direction.
- ~~Ask the agent conversationally and parse its reply~~ — **rejected on principle**, not merely unimplemented. It places an LLM inside the deterministic trading path, which Section 1 exists to prevent: a hallucinated or reformatted price would become a real order, and it adds seconds of latency to a pipeline built around regex parsing for microsecond determinism. Acceptable for a human asking "what are my positions?"; never for quotes or order placement.
- **Direct MCP client in the trading process, reusing the agent's cached OAuth token** — the remaining candidate. Keeps us on the official endpoint (satisfying Section 5's "no unofficial libraries"), but depends on the agent's token storage layout and on how refresh is handled, so it needs to be verified against the installed agent before being relied on.

**Robinhood quote provider (implemented 2026-07-25).** `execution.quote_source: robinhood` returns real bid/ask from the MCP. Three details from the live schemas shape the implementation:

- **Quotes take instrument UUIDs, not ticker/strike/expiry.** A naive quote is a three-call chain (`get_option_chains` → `get_option_instruments` → `get_option_quotes`). Since a contract's UUID is immutable, resolutions are cached permanently in the `option_instruments` table: the first quote for a contract costs two calls, every later one costs one. Without this the monitor would triple its broker usage for nothing.
- **Contract lookups must be unambiguous.** The filters (chain symbol, expiration, strike, type, active, tradable) should match exactly one contract. If more than one comes back the provider refuses rather than picking, because the wrong UUID is silently the wrong contract.
- **Markets that cannot be traded are rejected**, not passed through: a zero ask or a crossed book (bid > ask) usually means a halt or an illiquid strike, and decisioning on it is meaningless.

Every failure — auth, rate limit, ambiguity, malformed payload, unusable market — surfaces as `QuoteUnavailable`, so the signal is skipped. The provider never falls back to a simulated price.

**Order preconditions.** The `place_option_order` and `review_option_order` docs require the account be `agentic_allowed=true` **and** hold `option_level_2` or `option_level_3`, and state plainly not to call otherwise. `robinhood_mcp.account_is_tradeable()` checks this via `get_accounts` so a misconfigured account becomes a clean skip instead of a rejected order. Note the MCP supports **single-leg orders only** (Level 2 strategies) — which is exactly what this bot places — and `place_option_order` accepts a `ref_id` idempotency key, satisfying the double-submit requirement in this section.
...
- **Robinhood MCP status** — whether the broker integration is actually usable. Driven by whether the quote provider is *implemented* (`PROVIDER_META` in `src/services/quotes.py`), never by whether `quote_source` names it: a status light that went green because a config key said `robinhood` would invite the assumption that live trading works when it does not. The card also reports the supporting prerequisites — whether the Hermes Agent has an enabled `mcp_servers.robinhood` entry in `~/.hermes/config.yaml` (read-only, best-effort) and whether the user has a `robinhood_account_id` — so it shows what is still missing rather than a bare red light. States: `not_configured` / `agent_only` / `needs_login` (provider present but not authenticated, so it would fail closed on every quote) / `available` (authenticated but `quote_source` points elsewhere) / `active`. The `agent_only` state exists because the two are genuinely different: the Hermes Agent can hold a working OAuth session and answer position queries while the trading process — a separate process with no MCP client — still cannot route an order. Collapsing that into a flat "not integrated" reads as a bug to anyone who has just queried their positions through the agent, and obscures the fact that the agent session is the most likely bridge to wiring the trading path.
...
- **Dashboard users** are created with `scripts/add_user.py`, which prompts for the password without echoing it and stores only a scrypt hash. This replaces hand-written SQL: the previous `generate_password_hash('...')` one-liner placed the plaintext password in shell history, and pasting hashes by hand invites storing a wrong literal.
- **Robinhood credentials are per dashboard user.** `scripts/robinhood_login.py --user <name>` performs a one-time OAuth 2.1 PKCE authorization and caches tokens in `.robinhood_token.<user_id>.json` (mode `0600`, gitignored). Every broker call carries a `user_id` — there is deliberately no default, because a fallback would silently route one tenant's calls through another's broker login. Deliberately a separate registration from the Hermes Agent's: refresh tokens are typically single-use and rotating, so refreshing the agent's copy from this process would break the agent's own Robinhood access.
  - Contract UUIDs and quotes are still cached *globally*, because both are properties of the market rather than of an account — caching them per user would spend the shared API budget re-asking the same question.
  - A legacy process-wide `.robinhood_token.json` is adopted automatically when exactly one user exists; with several, its owner is unknowable, so it is left in place and each user re-authorizes.
...
- The send date is tracked **per user and persisted in `system_state`**, so users configured for different times each get theirs, and a restart after the send time does not mail it again. Holding that flag in memory was a real defect: systemd restarts on failure, so any restart past the summary time re-sent it.
- Non-trading days are skipped. A weekend or holiday produces a row of zeroes, which is noise rather than information; the poller already skips those days for the same reason.
- `get_daily_metrics(user_id, date_str)` honours `date_str`, so a summary missed during an outage can be regenerated for a past day rather than silently reporting today's figures under yesterday's heading.
