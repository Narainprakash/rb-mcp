...
- **Robinhood credentials belong to the process, not to a dashboard user.** `scripts/robinhood_login.py` performs a one-time OAuth 2.1 PKCE authorization and caches tokens in `.robinhood_token.json` (mode `0600`, gitignored). Deliberately a separate registration from the Hermes Agent's: refresh tokens are typically single-use and rotating, so refreshing the agent's copy from this process would silently break the agent's own Robinhood access. A consequence worth stating plainly — all dashboard users currently share this single broker login, which is why multi-user live trading is still gated (Section 16).
